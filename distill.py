...
def train(args):
    ...
    print(f"[distill] device={device} dtype={dtype} rank={args.rank} "
          f"steps={args.steps} smoke_test={args.smoke_test}")
    ...
    warmup_backbone = (3 if args.smoke_test
                       else min(args.warmup_backbone_steps, total_steps // 2))
    sched = get_cosine_schedule_with_warmup(
        optim, num_warmup_steps=max(1, total_steps // 20),
        num_training_steps=total_steps)

    # Guard #1: freeze the backbone for the warm-up window (train adapters only).
    student.set_backbone_requires_grad(False)
    backbone_frozen = True
    print(f"[distill] backbone frozen for first {warmup_backbone} steps")
    ...
    while step < total_steps:
        for batch in iter_batches(blocks, args.batch_size, shuffle=True,
            ...
            # cast teacher tensors to fp32 for a stable KL / MSE target
            ...
            # Guard #1 release: unfreeze the backbone after the warm-up window.
            if backbone_frozen and step >= warmup_backbone:
                student.set_backbone_requires_grad(True)
                backbone_frozen = False
                print(f"[distill] step {step}: backbone UNFROZEN")

            if step % args.log_every == 0:
                dt = time.time() - t0
                print(f"[distill] step {step:4d}/{total_steps} "
                      f"loss={loss.item():.4f} (kd={loss_kd.item():.4f} "
                      f"hidden={loss_h.item():.4f}) lr={sched.get_last_lr()[0]:.2e} "
                      f"{dt:.1f}s")
...
def build_argparser():
    ...
    ap.add_argument("--steps", type=int, default=300,
                    help="training steps (small by default — proof of concept)")
    ap.add_argument("--warmup-backbone-steps", type=int, default=200,
                    help="freeze backbone, train adapters only, for this many steps")
